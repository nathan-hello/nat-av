...
The reusable packages live in `./packages/core`, `./packages/drivers`, and
`./packages/plugins`. The Remix implementations live in `./examples/remix/`.

This repo should be considered a (very well thought out) beta project. The
`./packages/core` package is about 4k LOC, so please poke around and find things
to edit. The type system
seems overly complex, but that's mostly just because of the requirement of the RPC layer
found in `./packages/core/rpc/`.

If you are just curious and don't feel like doing a full `pnpm install`, just
look at the test files to learn how the library is used. You can run `pnpm test`
to verify that all the tests pass, but read
`./packages/drivers/cisco-roomos/README.md` first before running test because if
...
submodules at the very least, and the reusable packages can be consumed from a
Git submodule as demonstrated in `./examples/remix/git-submodule/`. I haven't
...
is implemented in `./packages/drivers/dante-router/` was written almost entirely
