...
describe("rpc roomos device", () => {
  it("calls roomos methods and receives roomos events through rpc", async () => {
    ...
    const off = await device.event.on("Bluetooth Streaming PlaybackPosition", (payload) => {
    ...
    const booking = await device.api.xCommand.Bookings.Get({ Id: "booking-123" });
    ...
    const subscription = await device.api.xFeedback.Bluetooth.Streaming.PlaybackPosition.subscribe();
    ...
    assert.deepEqual(transport.sent.map((message) => JSON.parse(message)), [
