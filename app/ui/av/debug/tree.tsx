...
export function DebugDeviceTree(handle: Handle<DebugDeviceTreeProps>) {
  return () => (
    <div mix={treeStyle}>
      {handle.props.tree.length > 0 ? handle.props.tree.map((node) => renderTreeNode(node, handle)) : <p mix={emptyStyle}>Waiting for device debug tree...</p>}
    </div>
  );
}
...
function renderTreeNode(node: DebugDeviceNode, handle: Handle<DebugDeviceTreeProps>, depth = 0) {
  ...
  return (
    <div key={node.name}>
      ...
      {node.children.length > 0 ? node.children.map((child) => renderTreeNode(child, handle, depth + 1)) : null}
    </div>
  );
}
